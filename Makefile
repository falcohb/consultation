...
fix: php-cbf php-insights-fix ## Fix coding standards
...
php-cs: ## Run php_codesniffer with PSR12 standard
	$(PHPQA) phpcs -v --ignore=*/src/Form/Model/* --standard=vendor/enabel/coding-standard-bundle/tools/phpcs.xml

php-cbf: ## Run PHP Code Beautifier and Fixer
	$(PHPQA) phpcbf -v --ignore=*/src/Form/Model/* --standard=vendor/enabel/coding-standard-bundle/tools/phpcs.xml
...
php-insights-fix: ## Run PHP Insights
	$(PHPQA) phpinsights --fix --flush-cache --no-interaction -v --config-path=phpinsights.php
